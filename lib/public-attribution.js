...
export function safeReferrer(value = "") {
  try {
    ...
    return ["http:", "https:"].includes(url.protocol) ? url.origin.slice(0, 200) : "";
  } catch {
    return "";
  }
}
...
export async function recordPublicConversion(supabase, eventName, { pack = "", placement = "", once = false } = {}) {
  if (typeof window === "undefined" || ["localhost", "127.0.0.1", "[::1]"].includes(window.location.hostname)) return { skipped: true };
