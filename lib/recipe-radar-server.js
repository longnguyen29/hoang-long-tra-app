...
export async function refreshRecipeRadarConcepts(admin, { triggeredBy = "" } = {}) {
  ...
  const rows = [...groups.entries()].map(([key, group]) => {
    ...
    // Testing and dismissed are explicit staff decisions. Candidate is an
    // algorithmic threshold and must be allowed to move when scoring improves.
    const lockedStage = prior && ["testing", "dismissed"].includes(prior.stage) ? prior.stage : score.suggestedStage;
    return {
...
export async function runRecipeRadar(admin, { mode = "manual", triggeredBy = "" } = {}) {
  ...
  try {
    ...
    // Search themes overlap by design, so the same article can appear in more than
    // one feed. PostgreSQL rejects duplicate conflict keys within one upsert.
    const uniqueSignals = dedupeSignals(collected.signals);
    const ids = uniqueSignals.map((item) => item.id);
    let existingIds = [];
    if (ids.length) {
      const { data } = await admin.from("recipe_radar_signals").select("id").in("id", ids);
      existingIds = (data || []).map((item) => item.id);
      const { error: signalError } = await admin.from("recipe_radar_signals").upsert(uniqueSignals, { onConflict: "id" });
      if (signalError) throw signalError;
    }
    const concepts = await refreshRecipeRadarConcepts(admin, { triggeredBy });
    const status = collected.errors.length && !uniqueSignals.length ? "failed" : collected.errors.length ? "partial" : "completed";
    const result = {
      status, queryCount: queries?.length || 0, signalCount: uniqueSignals.length,
