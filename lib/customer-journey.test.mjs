...
test('DNC removes sample and quote outreach actions while retaining accepted-order work',()=>{
 const result=buildCustomerJourney({opportunity:{id:'dnc',contact:'0903333841',prospect_suppressed:true,stage:'lead'},
 samples:[{id:'s',opportunity_id:'dnc',status:'new',ts:'2026-08-01'}],
 quotes:[{id:'q1',opportunity_id:'dnc',status:'draft',created_at:'2026-08-01'},{id:'q2',opportunity_id:'dnc',status:'accepted',accepted_at:'2026-08-02'}]});
 assert.deepEqual(result.actions.map(a=>a.kind),['order']);
 assert.equal(result.counts.samples,1);
});
test('explicit sample ownership overrides shared phone and legacy source hints',()=>{
 const result=buildCustomerJourney({opportunity:{id:'one',contact:'0903333841',source_id:'foreign'},samples:[
 {id:'foreign',opportunity_id:'two',phone:'0903333841',ts:'2026-08-01',status:'new'},
 {id:'owned',opportunity_id:'one',phone:'0903333899',ts:'2026-08-02',status:'new'},
 {id:'legacy',phone:'0903333841',ts:'2026-08-03',status:'new'}]});
 assert.equal(result.counts.samples,2);
 assert.equal(result.timeline.some(item=>item.id==='sample:foreign'),false);
});
const opportunity = {
