import {
  Home, Feather, Handshake, ShoppingBag, LayoutDashboard, Library, BookMarked,
  Mountain, Leaf, MapPin, ShieldCheck, Palette, Calendar, Sprout,
} from "lucide-react";
...
export const TOKENS = {
  ...
  // Jade-tinted shadows vanish on a near-black band; these two are the dark-band pair.
  hairlineOnDark: "rgba(217,184,118,0.22)",
  shadowOnDark: "0 10px 30px rgba(0,0,0,0.35)",
  radiusSm: 20,
  radius: 24,
  radiusLg: 28,
...
// Order matters and is deliberate: the two ways to buy come first, then the two ways to read
// about us, then the farms. Someone arriving to place an order should not have to pass the
// story to reach the shop.
export const NAV = [
  { id: "home", icon: Home, roles: ["admin", "wholesale", "retail"], label: { en: "Housepage", vi: "Trang chủ" } },
  { id: "retail", icon: ShoppingBag, roles: ["admin", "retail"], label: { en: "Shop", vi: "Cửa hàng" } },
  // "retail" is in the roles list (not just "wholesale") so logged-out/unapproved visitors can
  // still find Wholesale to sign up or apply — role no longer gates discovery, only
  // wholesaleAccount.wholesaleVerified gates the actual ordering UI inside the page.
  { id: "wholesale", icon: Handshake, roles: ["admin", "wholesale", "retail"], label: { en: "Wholesale", vi: "Dành cho Đối tác" } },
  ...
  // Farm goods from the North-West, sold by the House on behalf of the people who grow them.
  // Its own entry rather than a category inside Shop: the whole site frames itself as a tea
  // house, and a sack of rice filed under the tea list reads as an afterthought.
  { id: "goods", icon: Sprout, roles: ["admin", "wholesale", "retail"], label: { en: "Tây Bắc Goods", vi: "Nông sản Tây Bắc" } },
  // Not in the plain list — it has its own CTA at the foot of the side menu. Position here
  // only affects where it would appear if that ever changed.
  { id: "sessions", icon: Calendar, roles: ["admin", "wholesale", "retail"], label: { en: "Book a Session", vi: "Đặt Lịch Trà" } },
  { id: "frontdesk", icon: LayoutDashboard, roles: ["admin"], label: { en: "Dashboard", vi: "Bảng điều khiển" } },
];
...
// ---------- Order Flow (Front Desk › Orders › Board view) ----------
//
// A second, finer-grained field (orders.stage) sitting alongside the customer-facing
// STATUS_STEPS above — that one stays exactly as-is, so the public tracking page and its
// wording are untouched. This is the staff-only view of where an order actually is
// day-to-day. See supabase/migrations/0028_v3_order_flow.sql for the matching column/check.
export const ORDER_STAGES = [
  { id: "new_order", label: { en: "New Order", vi: "Đơn mới" } },
  { id: "confirm_details", label: { en: "Confirm Details", vi: "Xác nhận thông tin" } },
  { id: "prepare_materials", label: { en: "Prepare Materials", vi: "Chuẩn bị nguyên liệu" } },
  { id: "production", label: { en: "Production", vi: "Sản xuất / đóng gói" } },
  { id: "packing", label: { en: "Packing", vi: "Đóng gói" } },
  { id: "shipping", label: { en: "Shipping", vi: "Giao hàng" } },
  { id: "completed", label: { en: "Completed", vi: "Hoàn tất" } },
];

// Best-effort map from the old flat status to a starting stage — used only for legacy rows
// that predate this column (the migration backfills the database itself; this is the same
// mapping kept in JS as a defensive fallback in case a row is ever missing `stage`).
export function stageFromStatus(status) {
  return { pending: "new_order", confirmed: "confirm_details", shipped: "shipping", completed: "completed" }[status] || "new_order";
}

// Default checklist offered when an order first reaches a stage (staff can add/remove items
// per order from there — see the `stage_checklist` jsonb column). Deliberately generic: the
// house doesn't track real inventory counts yet, so this is a procedure checklist, not a
// stock system tied to real quantities.
export const STAGE_CHECKLIST_DEFAULTS = {
  confirm_details: [
    { en: "Confirm items, quantity, and delivery address", vi: "Xác nhận sản phẩm, số lượng và địa chỉ giao hàng" },
    { en: "Confirm price/tier and payment method", vi: "Xác nhận giá/mức chiết khấu và hình thức thanh toán" },
  ],
  prepare_materials: [
    { en: "Confirm tea grade & quantity on hand", vi: "Kiểm tra loại trà & số lượng tồn" },
    { en: "Prepare packaging (pouches/boxes)", vi: "Chuẩn bị bao bì (túi/hộp)" },
    { en: "Print labels", vi: "In nhãn" },
  ],
  production: [
    { en: "Weigh & pack to order", vi: "Cân & đóng gói theo đơn" },
    { en: "Quality check before sealing", vi: "Kiểm tra chất lượng trước khi niêm phong" },
  ],
  packing: [
    { en: "Seal & box, moisture-safe", vi: "Niêm phong & đóng thùng, chống ẩm" },
    { en: "Attach invoice/packing slip", vi: "Đính kèm hoá đơn/phiếu đóng gói" },
  ],
  shipping: [
    { en: "Book courier & get tracking code", vi: "Đặt vận chuyển & lấy mã vận đơn" },
    { en: "Photo of packed parcel", vi: "Chụp ảnh kiện hàng đã đóng gói" },
  ],
};

export const ISSUE_CATEGORIES = [
  { id: "material", label: { en: "Material", vi: "Nguyên liệu" } },
  { id: "customer", label: { en: "Customer", vi: "Khách hàng" } },
  { id: "documentation", label: { en: "Documentation", vi: "Giấy tờ" } },
  { id: "logistics", label: { en: "Logistics", vi: "Vận chuyển" } },
  { id: "other", label: { en: "Other", vi: "Khác" } },
];

// A starting playbook of problems that tend to come up, offered as tap-to-prefill suggestions
// when staff report a new issue — not a fixed list, any custom title can still be typed. Edit
// this directly as real patterns emerge; it's plain config, like the rest of the app's copy,
// not a database table (an *actual* flagged issue on an *actual* order lives in order_issues).
export const ISSUE_PLAYBOOK = [
  {
    category: "customer",
    title: { en: "Customer details mismatch", vi: "Thông tin khách hàng không khớp" },
    impact: "medium",
    suggestedFix: { en: "Verify the latest details with the customer before moving on.", vi: "Xác nhận lại thông tin mới nhất với khách trước khi tiếp tục." },
  },
  {
    category: "material",
    title: { en: "Tea or packaging material low/out of stock", vi: "Trà hoặc bao bì sắp hết/hết hàng" },
    impact: "high",
    suggestedFix: { en: "Check stock, substitute, or let the customer know about the delay.", vi: "Kiểm tra tồn kho, thay thế, hoặc báo khách về việc chậm trễ." },
  },
  {
    category: "documentation",
    title: { en: "VAT invoice / tax number missing", vi: "Thiếu mã số thuế / hoá đơn VAT" },
    impact: "medium",
    suggestedFix: { en: "Request the tax number and confirm invoicing needs before shipping.", vi: "Yêu cầu mã số thuế và xác nhận nhu cầu xuất hoá đơn trước khi giao." },
  },
  {
    category: "logistics",
    title: { en: "Delivery running late / due date at risk", vi: "Giao hàng trễ / có nguy cơ trễ hẹn" },
    impact: "high",
    suggestedFix: { en: "Contact the courier, update the customer proactively.", vi: "Liên hệ đơn vị vận chuyển, chủ động báo khách." },
  },
];
