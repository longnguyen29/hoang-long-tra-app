...
export function teaFitScore(value = "") {
  const text = String(value).toLocaleLowerCase("en");
  // Score against what Hoàng Long can actually make, including honest near
  // substitutions (leaf tea for matcha/hojicha/oolong concepts).
  if (/shan|black tea|green tea|jasmine|lotus|white tea|trà xanh|hồng trà|lục trà|hoa lài|hoa sen/.test(text)) return 98;
  if (/matcha|sencha|hojicha|oolong|genmaicha/.test(text)) return 88;
  const matches = TEA_WORDS.filter((word) => text.includes(word)).length;
  if (matches >= 2) return 92;
  if (matches === 1) return 84;
  if (/latte|mocktail|foam|fruit|sparkling|coconut|peach|citrus|cream/.test(text)) return 64;
  if (/cafe|beverage|menu/.test(text)) return 42;
  return 22;
}

export function feasibilityScore(value = "", category = "") {
  const text = String(value).toLocaleLowerCase("en");
  const recipeLike = /matcha|hojicha|genmaicha|oolong|latte|milk tea|sparkling|tonic|soda|mocktail|foam|cream|cheese|fruit|peach|mango|grape|berry|citrus|yuzu|lemon|coconut|pistachio|sesame|ube|tiramisu|kombucha|拿鐵|抹茶|焙茶|氣泡|奶蓋|水蜜桃|芒果|酪梨|布丁|ラテ|モクテル|クリーム|抹茶|ほうじ茶|라떼|말차|호지차|에이드|크림|ลาเต้|มัทฉะ|โซดา|ครีม/.test(text);
  let score = 75;
  if (!recipeLike && category === "menu-launch") score = 32;
...
export function scoreConcept(signals = [], now = new Date()) {
  ...
  // Recalculate from evidence each run so scoring policy improvements also fix
  // older saved signals instead of preserving stale scores indefinitely.
  const teaFit = clamp(Math.max(30, ...valid.map((item) => teaFitScore(`${item.title} ${item.excerpt || ""}`))));
  const feasibility = clamp(Math.round(valid.reduce((sum, item) => sum + feasibilityScore(`${item.title} ${item.excerpt || ""}`, item.category), 0) / Math.max(valid.length, 1)));
  // Product compatibility is the strongest signal: a global trend is only
  // useful when the House can translate it into one of its own teas.
  const total = Math.round(velocity * .20 + crossMarket * .20 + vietnamGap * .15 + teaFit * .30 + feasibility * .15);
...
export function toSignalRecord(item, { query, region, source = "google-news" } = {}) {
  ...
  const category = inferCategory(`${name} ${item.title}`);
  return {
    ...
    feasibility: feasibilityScore(`${name} ${item.title}`, query?.category || category),
  };
}
