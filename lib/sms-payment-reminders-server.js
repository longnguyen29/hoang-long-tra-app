...
function providerMessageId(reminderId) {
  return `hl_${createHash("sha256").update(String(reminderId || "")).digest("base64url").slice(0, 18)}`;
}
...
async function processOne(admin, reminder, { now, sendMessage }) {
  ...
  try {
    ...
    const result = await sendMessage({ id: providerMessageId(reminder.id), phone: eligibility.phone, text });
