...
test("uses the correct body separator for iPhone and Android", () => {
 ...
 test("tracking update uses saved carrier and code without claiming handover",()=>{
  const text=buildManualOrderMessage({order:{...order,trackingCode:"VT123",shippingCarrier:"viettel_post"},kind:"tracking"});
  assert.match(text,/VT123 \(Viettel Post\)/);
  assert.match(text,/\/don-hang\//);
  assert.doesNotMatch(text,/đã được gửi/);
  assert.equal(buildManualOrderMessage({order,kind:"tracking"}),"");
 });
