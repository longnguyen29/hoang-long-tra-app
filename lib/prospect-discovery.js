...
import { accountMetadata } from './prospect-types.js';
...
export function normalizeProspect(input) {
  ...
  if (!name || !url) throw new Error("Nhập tên khách hàng và đường dẫn nguồn hợp lệ.");
  return { ...accountMetadata(input), name, source_url: url, source_key: sourceKey(url), region: plain(input.region, 120), evidence: plain(input.evidence, 1500), evidence_kind: input.evidence_kind === "page_review" ? "page_review" : "search_snippet", contact: plain(input.contact, 240), notes: plain(input.notes, 2000) };
}
...
export function outreachDraft(p) {
  ...
  if ((p.account_type || "shop") !== "shop") return `Chào anh/chị ${plain(p.name, 160)}, mình là Long từ Trà Hoàng Long. Mình xin phép giới thiệu thông tin về trà Hoàng Long để anh/chị tham khảo. Nếu phù hợp, anh/chị có muốn nhận thêm thông tin không? Anh/chị chưa có nhu cầu thì mình xin phép không làm phiền thêm.`;
