...
export function buildCustomerJourney({
  ...
  const matchingSamples = samples.filter((item) => item.opportunity_id
    ? item.opportunity_id === opportunity.id
    : item.id === opportunity.source_id || (contactKey && normalizeContact(item.phone) === contactKey));
  ...
  // DNC suppresses new contact/preparation, while existing order fulfillment remains visible.
  const visibleActions=opportunity.prospect_suppressed
    ? actions.filter(action=>['order','order-risk','payment'].includes(action.kind)) : actions;
  return {
    opportunity,
