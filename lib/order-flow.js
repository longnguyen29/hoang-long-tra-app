...
// Older screens updated `status` without touching `stage`, while the ops console did the
// reverse. For those historical rows, never make an order appear to move backwards: use
// whichever field represents the furthest confirmed progress. Every new write updates both.
export function reconcileOrderStage(stage, status) {
  const storedStage = ORDER_STAGE_IDS.includes(stage) ? stage : "new_order";
  const statusStage = orderStageForStatus(status);
  return orderStageIndex(statusStage) > orderStageIndex(storedStage) ? statusStage : storedStage;
}
