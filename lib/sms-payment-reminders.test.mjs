...
test("gateway configuration fails closed without the encryption passphrase", () => {
  const config = readSmsGatewayConfig({
    SMS_GATEWAY_ACCESS_TOKEN: "restricted-token",
...
test("restricted access token is enough without account credentials", () => {
  const config = readSmsGatewayConfig({
    SMS_GATEWAY_ACCESS_TOKEN: "restricted-token",
    SMS_GATEWAY_DEVICE_ID: "device",
    SMS_GATEWAY_ENCRYPTION_PASSPHRASE: "correct horse battery staple",
  });
