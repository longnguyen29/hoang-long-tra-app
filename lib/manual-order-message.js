import {carrierLabel} from "./carrier-tracking.js";

export const MANUAL_MESSAGE_KINDS = [
  { id: "tracking", label: "Cập nhật mã vận đơn" },
...
export function buildManualOrderMessage({ order, kind, amountDue = 0, siteUrl = "https://www.hoanglongtra.com", now = new Date() }) {
  ...
  if (kind === "tracking") {
    ...
    return `Hoàng Long cập nhật đơn ${orderId}: mã vận đơn ${code} (${carrierLabel(carrier)}).${trackingUrl ? ` Theo dõi đơn tại ${trackingUrl}.` : ""} Cảm ơn quý khách.`;
  }
