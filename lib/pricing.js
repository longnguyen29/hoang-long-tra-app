...
export function calculatePricing(rawInputs = {}) {
  ...
  const currentPricePerPack = Math.max(0, number(inputs.currentPricePerPack));
  const currentPriceInclVatPerKg = currentPricePerPack / packKg;
  ...
  if (currentPricePerPack > 0 && currentPriceInclVatPerKg < safePartnerInclVatPerKg) {
