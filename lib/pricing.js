...
export function calculatePricing(rawInputs = {}) {
  ...
  const marginAtPrice = (priceInclVatPerKg) => {
    const revenueExVatPerKg = number(priceInclVatPerKg) / (1 + vatRate);
    return revenueExVatPerKg > 0
      ? ((revenueExVatPerKg * (1 - channelRate) - trueCostPerKg) / revenueExVatPerKg) * 100
      : null;
  };
  const profitAtPrice = (priceInclVatPerKg) => {
    const revenueExVatPerKg = number(priceInclVatPerKg) / (1 + vatRate);
    return revenueExVatPerKg * (1 - channelRate) - trueCostPerKg;
  };
  ...
  const currentMarginPercent = marginAtPrice(currentPriceInclVatPerKg);
  const partnerRevenueExVatPerKg = b2b.chargedInclVatPerKg / (1 + vatRate);
  const b2bMarginPercent = marginAtPrice(b2b.chargedInclVatPerKg);
  const retailMarginPercent = marginAtPrice(retail.listInclVatPerKg);
  const b2bProfitPerKg = profitAtPrice(b2b.chargedInclVatPerKg);
  const retailProfitPerKg = profitAtPrice(retail.listInclVatPerKg);
  const b2bProfitPerOrder = b2bProfitPerKg * orderKg;
  const retailProfitPerOrder = retailProfitPerKg * orderKg;
  ...
  return {
    ...
    b2bMarginPercent: Math.round(number(b2bMarginPercent) * 10) / 10,
    retailMarginPercent: Math.round(number(retailMarginPercent) * 10) / 10,
    b2bProfitPerKg: roundMoney(b2bProfitPerKg),
    retailProfitPerKg: roundMoney(retailProfitPerKg),
    b2bProfitPerOrder: roundMoney(b2bProfitPerOrder),
    retailProfitPerOrder: roundMoney(retailProfitPerOrder),
    profitPerKg: roundMoney(b2bProfitPerKg),
    profitPerOrder: roundMoney(b2bProfitPerOrder),
    breakEvenPacks,
    warnings,
  };
}
