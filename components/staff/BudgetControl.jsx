      ...
      {periodDraft && (
        <Dialog title="Tạo kỳ ngân sách" onClose={() => setPeriodDraft(null)}>
          <form className={styles.form} onSubmit={savePeriod}>
            ...
            <label><span>Trần ngân sách</span><input required type="number" min="0" step="1000" value={periodDraft.totalLimit} onChange={(event) => setPeriodDraft({ ...periodDraft, totalLimit: event.target.value })} /><small>Tổng tiền tối đa có thể phân bổ trong kỳ.</small></label>
      ...
      {allocationDraft && (
        <Dialog title="Xin phân bổ ngân sách" onClose={() => setAllocationDraft(null)}>
          <form className={styles.form} onSubmit={saveAllocation}>
            ...
            <div className={styles.formGrid}><label><span>Loại chi</span><select value={allocationDraft.spendKind} onChange={(event) => setAllocationDraft({ ...allocationDraft, spendKind: event.target.value })}><option value="recurring">Chi thường xuyên</option><option value="one_off">Đầu tư một lần</option><option value="reserve">Dự phòng</option></select><small>&nbsp;</small></label><label><span>Số tiền</span><input required type="number" min="1" step="1000" value={allocationDraft.amount} onChange={(event) => setAllocationDraft({ ...allocationDraft, amount: event.target.value })} /><small>&nbsp;</small></label></div>
