"use client";
// Development-only fixture. All persistence is denied; no customer database is used.
...
export default function StaffReview() {
 ...
 return <>
  ...
  {orderOpen&&<OrderDetail onDelete={async()=>({error:"Đây là bản minh họa. Không xoá dữ liệu thật."})} order={{id:'REVIEW-001',customerName:'Quán minh họa',contact:'review@example.test',stage:'confirm_details',health:'on_track',estimatedTotal:1000000,lines:[{name:{vi:'Trà minh họa'},qty:2,unit:'kg',price:500000}]}} events={[]} costs={[]} receivable={{id:'review-payment',status:'partial',total:1000000,paid:300000}} loadingEvents={false} saving={false} healthDraft="on_track" setHealthDraft={()=>{}} waitingDraft="us" setWaitingDraft={()=>{}} healthNoteDraft="" setHealthNoteDraft={()=>{}} trackingDraft="" setTrackingDraft={()=>{}} carrierDraft="" setCarrierDraft={()=>{}} onUpdate={async()=>false} onIssueReceivable={async()=>false} onRecordPayment={async()=>false} onAddCost={async()=>false} onSyncCosts={async()=>false} onDeleteCost={async()=>false} onClose={()=>setOrderOpen(false)}/>}
 </>;
}
