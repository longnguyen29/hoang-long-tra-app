...
export default function HouseJournal(){
 const {locale:lang,toggleLocale}=useLocale();const [images,setImages]=useState([]),[articles,setArticles]=useState([]),[active,setActive]=useState(null),[lightbox,setLightbox]=useState(null);const supabase=useMemo(()=>createClient(),[]),t=COPY[lang],local=v=>{const text=v?.[lang]||v?.en||v?.vi||"";return text.trim()==="100% Natural"?(lang==="vi"?"Hoa lài":"Jasmine flowers"):text};
 ...
 const articleRef=useRef(null),imageRef=useRef(null);
 useDialogFocus(articleRef,Boolean(article),()=>setActive(null));
 useDialogFocus(imageRef,Boolean(lightbox),()=>setLightbox(null));
 ...
 <section className={styles.gallery}><header><h2>{t.images}</h2><span>{String(images.length).padStart(2,"0")}</span></header>{images.length?<div>{images.map((g,i)=><figure key={g.id}><button className={styles.galleryImageButton} aria-label={local(g.caption)||(lang==="vi"?`Mở ảnh ${i+1}`:`Open image ${i+1}`)} onClick={()=>setLightbox(g)}><img src={g.url} alt={local(g.caption)} loading={i>1?"lazy":undefined}/></button>{local(g.caption)&&<figcaption>{local(g.caption)}</figcaption>}</figure>)}</div>:<p>{t.emptyImages}</p>}</section>
 ...
 {article&&<div ref={articleRef} tabIndex={-1} aria-label={local(article.title)} className={styles.articleOverlay} role="dialog" aria-modal="true"><header><button onClick={()=>setActive(null)}><X size={18}/>{t.close}</button></header><article><p>{category(article.category)}</p><h2>{local(article.title)}</h2><div>{local(article.body)}</div></article></div>}
 {lightbox&&<div ref={imageRef} tabIndex={-1} aria-label={lang==="vi"?"Xem ảnh":"Image viewer"} className={styles.lightbox} role="dialog" aria-modal="true" onClick={()=>setLightbox(null)}><button aria-label={lang==="vi"?"Đóng ảnh":"Close image"}><X/></button><figure><img src={lightbox.url} alt={local(lightbox.caption)}/>{local(lightbox.caption)&&<figcaption>{local(lightbox.caption)}</figcaption>}</figure></div>}
 </main>
}
