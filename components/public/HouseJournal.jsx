...
const COPY={vi:{back:"Nhà Hoàng Long",switcher:"EN",eyebrow:"Thư viện sống",title:"Hình ảnh, ghi chép và kiến thức từ đời sống của trà.",intro:"Một thư viện không cố biến mọi thứ thành nội dung. Chỉ giữ lại những gì giúp ta nhìn lá trà, nơi chốn và cách pha rõ hơn.",images:"Từ xưởng và vùng trà",reading:"Sổ đọc về trà",emptyImages:"Những hình ảnh đầu tiên đang được chuẩn bị.",emptyReading:"Những bài đọc đầu tiên đang được chuẩn bị.",read:"Mở bài",close:"Đóng bài",closing:"Đọc ít hơn. Nhìn kỹ hơn."},en:{back:"House of Hoàng Long",switcher:"VI",eyebrow:"A living library",title:"Images, field notes and knowledge from the life of tea.",intro:"A library that does not turn everything into content. It keeps only what helps us see the leaf, the place and the brew more clearly.",images:"From the factory and tea country",reading:"The tea reading room",emptyImages:"The first images are being prepared.",emptyReading:"The first readings are being prepared.",read:"Open article",close:"Close article",closing:"Read less. Look more closely."}};
export default function HouseJournal(){
 const [lang,setLang]=useState("vi"),[images,setImages]=useState([]),[articles,setArticles]=useState([]),[active,setActive]=useState(null),[lightbox,setLightbox]=useState(null);const supabase=useMemo(()=>createClient(),[]),t=COPY[lang],local=v=>v?.[lang]||v?.en||v?.vi||"";
 ...
 return <main className={styles.page}><header className={styles.header}><Link href="/"><ArrowLeft size={17}/>{t.back}</Link><button onClick={()=>setLang(lang==="vi"?"en":"vi")}><Globe2 size={15}/>{t.switcher}</button></header>
