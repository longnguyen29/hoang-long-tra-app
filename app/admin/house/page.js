...
import HouseControl from "@/components/staff/HouseControl";

export default function HouseAdminPage(){
 const router=useRouter(),supabase=useMemo(()=>createClient(),[]);const [status,setStatus]=useState("checking"),[email,setEmail]=useState("");
 ...
 if(status==="checking")return <main className="hl-admin-state" aria-live="polite"><span className="hl-admin-state__pulse"/><p>Đang mở không gian quản trị…</p></main>;
 ...
 return <HouseControl supabase={supabase} email={email} onLogout={logout}/>;
}
